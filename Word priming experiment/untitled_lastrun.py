...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/apple/Documents/PSY310/7. Word priming experiment/untitled_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    trials = data.TrialHandler(nReps=1.0, method='random', 
        ...
        trialList=data.importConditions('/Users/apple/Downloads/study words.xlsx'),
    ...
    trials_2 = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('/Users/apple/Downloads/test_word.xlsx'),
    ...
    for thisTrial_2 in trials_2:
        ...
        # keep track of which components have finished
        testComponents = [Test_word, textbox, submit_text, testmouse]
        ...
        while continueRoutine:
            ...
            # if Test_word is starting this frame...
            if Test_word.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                ...
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Test_word.started')
                # update status
                Test_word.status = STARTED
                Test_word.setAutoDraw(True)
            
            # if Test_word is active this frame...
            if Test_word.status == STARTED:
                # update params
                pass
            
            # if Test_word is stopping this frame...
            if Test_word.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Test_word.tStartRefresh + 10-frameTolerance:
                    # keep track of stop time/frame for later
                    Test_word.tStop = t  # not accounting for scr refresh
                    Test_word.tStopRefresh = tThisFlipGlobal  # on global time
                    Test_word.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'Test_word.stopped')
                    # update status
                    Test_word.status = FINISHED
